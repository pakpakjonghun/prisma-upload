...
import { deleteFilesWithTransaction } from "../file/delete";
...
export const deleteDocumentsAndFiles = async (documentIds: string[]): Promise<void> => {
    ...
    try {
        ...
        await fileChainingPrisma.$transaction(async (tx) => {
            ...
            filePathList.push(...documents.map((doc) => {
                if (doc.file) {
        ...
        // 파일 삭제
        await Promise.all(filePathList.map(async (p) => {
